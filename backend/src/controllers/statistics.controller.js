...
export const GetHistogrma = async (req, res) => {
    try {
            ...
            "SELECT COUNT(*) as No, class.Id, class.Title, user.Name  FROM class INNER JOIN user ON user.Id = class.Teacher_Id INNER JOIN section ON section.Class_Id = class.Id GROUP BY class.Id  ORDER BY No DESC LIMIT 5;"
            );
            console.log(row[0])
